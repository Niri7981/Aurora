...
```text
当前状态：全局搜索已重做；统计全部与关键词匹配语义已分开
当前任务：修复 AND/OR 误命中和无法统计全部消息的问题（已完成）
下一步：由用户重启 Gateway，验证全部消息总数和关键词匹配模式
...
- [x] 使用 `content_urls` 保存消息包含的多个网址及其类型
- [x] 兼容并回填旧 `external_url`，不重写消息正文
- [x] 全局搜索覆盖 `content_urls`，结果中展示网址类型和地址
- [x] 返回 `total_matches` 和分来源准确计数，不把单页数量当作总数
