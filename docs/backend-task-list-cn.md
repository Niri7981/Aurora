...
```text
当前状态：第一个闭环 / Milestone 1 / 任务 1.1 已完成，等待复核
当前任务：1.1 引入 PostgreSQL 客户端依赖和示例连接配置（已完成）
下一步：1.2 创建空迁移目录并建立 PgPool
...
- [x] 使用本机 PostgreSQL，不依赖 Docker
- [x] 当前开发环境为 PostgreSQL 15
...
- [ ] 实现创建 Analysis Scope
- [ ] 实现按 Scope 和游标分页读取消息
- [ ] 增加真实 PostgreSQL 集成测试

## Milestone 4：标准聊天导入

目标：先通过稳定的 Aurora 标准格式跑通导入，不被微信格式差异阻塞。

- [ ] 定义 Aurora 标准聊天 JSON 格式
- [ ] 准备一份不含私人内容的测试样例
- [ ] 实现 JSON 解析与校验
- [ ] 实现 TXT 最小解析格式
- [ ] 实现导入预览
