...
当前任务：增加经 MCP 独立用户确认后应用提案的工具（已完成）
下一步：增加拒绝提案入口，并记录用户决定的审计事件
现在不要做：由 Agent 传入批准参数、绕过用户确认直接写入、管理页面
```
...
Agent 只能提出修改建议，不能批准自己的建议。首版由 MCP 客户端向用户发起独立确认，
确认后由 Aurora 校验版本并写入；不依赖管理页面。

```text
Agent 通过 MCP 创建 pending 提案
    -> Agent 可以通过 MCP 请求应用
    -> MCP 客户端独立询问用户，Agent 不能代替用户确认
    -> 用户确认、拒绝或取消
    -> 批准时校验原文件版本并写入
